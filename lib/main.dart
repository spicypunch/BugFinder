...
class _CameraScreenState extends State<CameraScreen>
  ...
  final InspectionHistoryRepository _historyRepository =
  ...
  Future<void> _initializeCamera() async {
    ...
    try {
      ...
      _showErrorDialog(AppLocalizations.of(context)!
          .cameraInitializationFailed(e.toString()));
    }
  }
    ...
    await Navigator.of(context).push(
      MaterialPageRoute<void>(
        builder: (BuildContext context) {
          return HistoryScreen(repository: _historyRepository);
        },
      ),
    );
  }

  Future<InspectionRecord?> _showCaptureResultSheet({
    required DateTime capturedAt,
    required String originalImagePath,
    required String filteredImagePath,
  }) {
    return showModalBottomSheet<InspectionRecord>(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (BuildContext context) {
        return CaptureResultSheet(
  ...
  Future<void> _capturePhoto() async {
    ...
    try {
      ...
      final Uint8List? filteredImageBytes =
          applyNegativeFilter(originalImageBytes);
      final bool usedFallback = filteredImageBytes == null;
      final File filteredFile = usedFallback
          ? photoFile
          : File(
      ...
      _showToast(
        usedFallback
            ? AppLocalizations.of(context)!.fallbackImageSaved
            : AppLocalizations.of(context)!.imageSavedToGallery,
      );

      final InspectionRecord? savedRecord = await _showCaptureResultSheet(
        capturedAt: capturedAt,
      ...
      if (savedRecord != null) {
        setState(() {
          _lastCapturedImagePath = savedRecord.filteredImagePath;
        });
        _showToast(AppLocalizations.of(context)!.recordSaved);
        await _deleteFileIfExists(photoFile.path);
        if (!usedFallback) {
          await _deleteFileIfExists(filteredFile.path);
        }
                        ...
                        Row(
                          ...
                          children: [
                            IconButton(
                              ...
                              tooltip:
                                  AppLocalizations.of(context)!.historyTooltip,
                            ),
                            IconButton(
                              onPressed:
                                  cameras.length > 1 ? _switchCamera : null,
                              icon: const Icon(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        if (_lastCapturedImagePath != null &&
                            File(_lastCapturedImagePath!).existsSync())
                          Container(
                            width: 50,
                            height: 50,
